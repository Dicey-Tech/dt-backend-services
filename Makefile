...
isort_check: ## check that isort has been run
	isort --check-only -rc dt_talenthub/

isort: ## run isort to sort imports in all Python files
	isort --recursive --atomic dt_talenthub/

style: ## run Python style checker
	pylint --rcfile=pylintrc dt_talenthub *.py
...
pii_check: ## check for PII annotations on all Django models
	DJANGO_SETTINGS_MODULE=dt_talenthub.settings.test \
...
dummy_translations: ## generate dummy translation (.po) files
	cd dt_talenthub && i18n_tool dummy
...
open-devstack: ## open a shell on the server started by start-devstack
	docker exec -it dt-talenthub /edx/app/dt-talenthub/devstack.sh open

pkg-devstack: ## build the dt-talenthub image from the latest configuration and code
	docker build -t dt-talenthub:latest -f docker/build/dt-talenthub/Dockerfile git://github.com/edx/configuration

detect_changed_source_translations: ## check if translation files are up-to-date
	cd dt_talenthub && i18n_tool changed

validate_translations: fake_translations detect_changed_source_translations ## install fake translations and check if translation files are up-to-date

docker_build:
	docker build . -f Dockerfile -t openedx/dt-talenthub
	docker build . -f Dockerfile --target newrelic -t openedx/dt-talenthub:latest-newrelic

travis_docker_tag: docker_build
	docker tag openedx/dt-talenthub openedx/dt-talenthub:$$TRAVIS_COMMIT
	docker tag openedx/dt-talenthub:latest-newrelic openedx/dt-talenthub:$$TRAVIS_COMMIT-newrelic

travis_docker_auth:
	echo "$$DOCKER_PASSWORD" | docker login -u "$$DOCKER_USERNAME" --password-stdin

travis_docker_push: travis_docker_tag travis_docker_auth ## push to docker hub
	docker push 'openedx/dt-talenthub:latest'
	docker push "openedx/dt-talenthub:$$TRAVIS_COMMIT"
	docker push 'openedx/dt-talenthub:latest-newrelic'
	docker push "openedx/dt-talenthub:$$TRAVIS_COMMIT-newrelic"
...
app-shell: # Run the app shell as root
	docker exec -u 0 -it dt_talenthub.app bash

db-shell: # Run the app shell as root, enter the app's database
	docker exec -u 0 -it dt_talenthub.db mysql -u root dt_talenthub
...
%-attach:
	docker attach dt_talenthub.$*

github_docker_build:
	docker build . -f Dockerfile --target app -t openedx/dt-talenthub
	docker build . -f Dockerfile --target newrelic -t openedx/dt-talenthub:latest-newrelic

github_docker_tag: github_docker_build
	docker tag openedx/dt-talenthub openedx/dt-talenthub:${GITHUB_SHA}
	docker tag openedx/dt-talenthub:latest-newrelic openedx/dt-talenthub:${GITHUB_SHA}-newrelic
...
github_docker_push: github_docker_tag github_docker_auth ## push to docker hub
	docker push 'openedx/dt-talenthub:latest'
	docker push "openedx/dt-talenthub:${GITHUB_SHA}"
	docker push 'openedx/dt-talenthub:latest-newrelic'
	docker push "openedx/dt-talenthub:${GITHUB_SHA}-newrelic"
